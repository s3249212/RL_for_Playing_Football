...
float Neural_network::activationFunction(float input){
return input;
}

float Neural_network::dActivationFunction(float input){
return 1;
}
...
layerSizes(layerSizes)
{
    ...
    for(int i = 0; i < layerSizes.size(); i++){
        ...
        if(i < layerSizes.size() - 1){
            weights[i] = new float*[layerSizes.at(i) + 1];
            for(int j = 0; j < layerSizes.at(i) + 1; j++){
                weights[i][j] = new float[layerSizes.at(i + 1)];
                for(int k = 0; k < layerSizes.at(i + 1); k++){
                    weights[i][j][k] = minInit + static_cast <float> (rand()) / (static_cast <float> (RAND_MAX / (maxInit - minInit)));
                }
            }
        }
    }
...
vector<float> Neural_network::forwardPass(vector<float> input){
...
void Neural_network::backwardPass(vector<float> targets){
